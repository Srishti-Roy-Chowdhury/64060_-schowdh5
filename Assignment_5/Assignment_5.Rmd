...
```{r}
#From the lecture slides, we learned that agnes ( ) and hclust ( ) functions behave very similarly, so I am using ward linkage here to find the cluster number based on euclidean distance matrix
hc.ward <- hclust(dist, method="ward")
plot(hc.ward, cex=0.6, hang=-1)
...
#****I couldn't solve the next part. I think this part is not covered in the class****#
```
...
#Finally, I am in support of normalizing the dataset. Otherwise Variables with larger numerical ranges (e.g., sodium, potassium) would influence the distance calculations.
```
